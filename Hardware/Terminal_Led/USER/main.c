    ...
    while (1) {
        ...
        switch (key) {
            case 0:
                ...
                break;
            case 1:
                while (key1 == 0 || (key1 == 1 && key1CD < KEYCD_TIME))
                    ;
                if (PWMval[0] == 0)
                    PWMval[0] = 50;
                else
                    PWMval[0] = 0;
                TIM_SetCompare[0](TIM3, PWMval[0] * 5);
                key1CD = 0;
                break;
            case 2:
                while (key2 == 0 || (key2 == 1 && key2CD < KEYCD_TIME))
                    ;
                if (PWMval[1] == 0)
                    PWMval[1] = 50;
                else
                    PWMval[1] = 0;
                TIM_SetCompare[1](TIM3, PWMval[1] * 5);
                key2CD = 0;

                break;
            case 3:
                while (key3 == 0 || (key3 == 1 && key3CD < KEYCD_TIME))
                    ;
                if (PWMval[2] == 0)
                    PWMval[2] = 50;
                else
                    PWMval[2] = 0;
                TIM_SetCompare[2](TIM3, PWMval[2] * 5);
                key3CD = 0;

                break;
            case 4:
                while (key4 == 0 || (key4 == 1 && key4CD < KEYCD_TIME))
                    ;
                if (PWMval[3] == 0)
                    PWMval[3] = 50;
                else
                    PWMval[3] = 0;
                TIM_SetCompare[3](TIM3, PWMval[3] * 5);
                key4CD = 0;
                break;
        }
    }
            ...
            if (zigbee->terminalOnlineFlag) {
			  ...
			  set_LED1;
                ...
                if (xSemaphoreTake(infoFlag_semaphore, 30000) == pdPASS) {
                    // 如果有入网申请需要发送
                    HBPack[0] = 0x01; // 这里一定要修改为对应终端的设备类型码
...
static void AnalyseCommand_Task(void *parameter)
{
    ...
    while (1) {
        ...
        if (command.buffer[0] == 0x55) // Zigbee反馈命令
          ...
          if(command.buffer[3] == 0x01){ // 这里一定要修改为对应终端的设备类型码
            if(command.buffer[6] == 0x00){
              // 亮度调节
              PWM_LED_Light_Set(command.buffer[5], command.buffer[7]);
            }
            // // 更改工作模式
            // else if(command.buffer[6] == 0x01){
            //   PWM_LED_Mode(command.buffer[5], 0);
            // }
            // else if(command.buffer[6] == 0x02){
            //   PWM_LED_Mode(command.buffer[5], 1);
            // }

        }
        else if(command.buffer[3] == 0x00){
          if(command.buffer[5] == 0x01)
            zigbee->terminalOnlineFlag = 1;
        }
        else if(command.buffer[3] == 0xFF){
...
void TIM2_IRQHandler(void)
{
    ...
    if (TIM_GetITStatus(TIM2, TIM_IT_Update) == SET) { // 溢出中断
        ...
        if (key1CD < KEYCD_TIME) key1CD++;
        if (key2CD < KEYCD_TIME) key2CD++;
        if (key3CD < KEYCD_TIME) key3CD++;
        if (key4CD < KEYCD_TIME) key4CD++;
