...
void Zigbee_Open_Net(Zigbee *zigbee){
	uint8_t i;
	uint8_t NetStart[] = {0x55, 0x04, 0x00, 0x02, 0x00, 0x02,};
...
/**
 ...
 void Zigbee_Update_TerminalOnlineFlag(Zigbee *zigbee){
  ...
  uint8_t HBPack[1 + 2 + 8 + 1];
	 uint8_t i = 0;
  HBPack[i++] = 0x01; // 这里一定要修改为对应终端的设备类型码
  ...
  for(uint8_t j = 0; j < 4; j++)
	...
	while(zigbee->ackFlag != 1){
		send_customFrame(zigbee, 0xFE,1 + 2 + 1+ 8 ,HBPack);
		delay(700);//考虑数据接收延迟,避免频繁发送导致中控数据拥堵
		set_LED1;
		if(wait_count >= 3){//3s发三次没收到应答，直接退出，表示为没有入网
...
void Zigbee_Analyse_Command_Data(Zigbee *zigbee){
	if (model == E18){
		if(usart1_rx_buffer[2] == 0x00 && usart1_rx_buffer[3] == 0x00){//55 2A 00 00 00
			if(usart1_rx_buffer[4] == 0x00){//已配网获取一波设备信息
			...
			else if(usart1_rx_buffer[4] == 0xFF){//未组网
				zigbee->LAddr[0] = usart1_rx_buffer[6];
				zigbee->LAddr[1] = usart1_rx_buffer[7];
				zigbee->LAddr[2] = usart1_rx_buffer[8];
				zigbee->LAddr[3] = usart1_rx_buffer[9];
		...
		else if(usart1_rx_buffer[1] == 0x04 && usart1_rx_buffer[2] == 0x00 && usart1_rx_buffer[3] == 0x02 && usart1_rx_buffer[4] == 0x00 && usart1_rx_buffer[5] == 0x02){//55 04 00 02 00 02
			zigbee->openNetFlag = 1;//判断网络已打开
		}
		else if(usart1_rx_buffer[1] == 0x04 && usart1_rx_buffer[2] == 0x00 && usart1_rx_buffer[3] == 0x05 && usart1_rx_buffer[4] == 0x00 && usart1_rx_buffer[5] == 0x05){//55 04 00 05 00 05
			zigbee->setTypeFlag = 1;//判断成功设置终端类型
		}
		else if(zigbee->readySetTargetFlag == 1 && usart1_rx_buffer[1] == 0x04 && usart1_rx_buffer[2] == 0x00 && usart1_rx_buffer[3] == 0x11 && usart1_rx_buffer[4] == 0x00 && usart1_rx_buffer[5] == 0x11){//55 04 00 11 00 11
			zigbee->modeFlag = 1;
		}
		else if(zigbee->readySetTargetFlag == 0 && usart1_rx_buffer[1] == 0x04 && usart1_rx_buffer[2] == 0x00 && usart1_rx_buffer[3] == 0x11 && usart1_rx_buffer[4] == 0x00 && usart1_rx_buffer[5] == 0x11){//55 04 00 11 00 11
			if(zigbee->setSendTargetFlag == 0) zigbee->setSendTargetFlag = 1;
			else zigbee->setSendTargetFlag = 0;
		}
		else if(usart1_rx_buffer[1] == 0x03 && usart1_rx_buffer[2] == 0xFF && usart1_rx_buffer[3] == 0xFE && usart1_rx_buffer[4] == 0x01){//55 03 FF FE 01
			if(zigbee->modeFlag != 0) zigbee->modeFlag = 0;
		}
	}
	else if(model == E180){
