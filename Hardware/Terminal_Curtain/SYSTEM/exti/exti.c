...
void EXTI0_IRQHandler(void){
    if(EXTI_GetITStatus(EXTI_Line0)!=RESET){//判断某个线上的中断是否发生
		key1CD = MilliSecond+20;
		while(key1CD > MilliSecond){
			if(PAin(0) == 0) {			
...
void EXTI1_IRQHandler(void){
    if(EXTI_GetITStatus(EXTI_Line1)!=RESET){//判断某个线上的中断是否发生
		key2CD = MilliSecond+20;
		while(key2CD > MilliSecond){
			if(PAin(1) == 0) {			
				EXTI_ClearITPendingBit(EXTI_Line1); //清除 LINE 上的中断标志位
				return;
			}
		}
		if(key2CD <= MilliSecond){
			if(PWMval[1] == 0){
				LED_Open(1);
			}
			else{
...
void EXTI2_IRQHandler(void){
    if(EXTI_GetITStatus(EXTI_Line2)!=RESET){//判断某个线上的中断是否发生
		key3CD = MilliSecond+20;
		while(key3CD > MilliSecond){
			if(PAin(2) == 0) {			
				EXTI_ClearITPendingBit(EXTI_Line2); //清除 LINE 上的中断标志位
				return;
			}
		}
		if(key3CD <= MilliSecond){
			if(PWMval[2] == 0){
				LED_Open(2);
			}
			else{
...
void EXTI3_IRQHandler(void){
    if(EXTI_GetITStatus(EXTI_Line3)!=RESET){//判断某个线上的中断是否发生
		key4CD = MilliSecond+20;
		while(key4CD > MilliSecond){
			if(PAin(3) == 0) {			
				EXTI_ClearITPendingBit(EXTI_Line3); //清除 LINE 上的中断标志位
				return;
			}
		}
		if(key4CD <= MilliSecond){
			if(PWMval[3] == 0){
				LED_Open(3);
			}
			else{
