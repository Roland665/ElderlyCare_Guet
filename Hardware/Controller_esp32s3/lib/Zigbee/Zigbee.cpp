...
void Zigbee::change_mode(uint8_t mode){
  ...
  uint8_t count = 0;
	if(mode == 0){
		//进入配置模式
    ESP_LOGD(TAG, "Changing mode to mode 0");
		while(m_modeFlag != 0){
			m_serial->write(change_mode0_command, 3);
      ESP_LOGV(TAG, ".");
			wait;
      count++;
      if(count%5 == 0){ // 疑似串口有概率卡死在这，每5次就多等3个wait，10次还不行就重启
        if(count == 10)
          esp_restart();
			  wait;
			  wait;
    ...
    ESP_LOGD(TAG, "Zigbee module has entered mode 0");
	}
	else if(mode == 1){
		//进入透传模式
    ESP_LOGD(TAG, "Changing mode to mode 1");
    while(m_modeFlag != 1){
			m_serial->write(change_mode1_command, 9);
      ESP_LOGV(TAG, ".");
      wait;
      count++;
      if(count >= 5){ // 防止在收到“成功进入透传模式”的反馈前先收到了透传消息，导致反馈被丢弃然后卡死在这个循环
			  wait;
        m_modeFlag = 1;
        change_mode(0);
        count = 0;
    ...
    ESP_LOGD(TAG, "Zigbee module has entered mode 1");
	}
}
...
void Zigbee::get_state(void){
  ...
  ESP_LOGD(TAG, "Checking zigbee model state");
  uint8_t count = 0;
...
void Zigbee::set_send_target(uint8_t *DSAddr,uint8_t DPort){
  ...
  ESP_LOGD(TAG, "Setting zigbee sending direction short address: %x %x", DSAddr[0], DSAddr[1]);
	while(m_setTargetStep == 0){
		m_serial->write(setDirShortAddr_command, 10);
    ESP_LOGV(TAG, ".");
    wait;
	}
  ESP_LOGD(TAG, "Setting zigbee sending direction port");
	//设置目标端口
...
void Zigbee::analyze_feedback(uint8_t *buffer){
  ESP_LOGD(TAG, "Analyzing zigbee model feedback");
  if(buffer[1] == 0x2A && buffer[2] == 0x00 && buffer[3] == 0x00 && buffer[4] == 0x00 && buffer[5] == 0x00){//55 2A 00 00 00 00
    // 查询状态返回数据
...
// 若想透传数据，则使用此函数前一定要调用 change_mode(1)；若是想配置Zigbee模块，则一定要调用 change_mode(0)。
